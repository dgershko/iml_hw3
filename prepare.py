...
def transform_features(new_data, transform_to_angle=False):
    ...
    transformed_data["current_location_x"] = transformed_data["current_location"].apply(lambda location: ast.literal_eval(location)[0]).astype(float)
    transformed_data["current_location_y"] = transformed_data["current_location"].apply(lambda location: ast.literal_eval(location)[1]).astype(float)
    ...
    if transform_to_angle:
        transformed_data = transform_to_angle(transformed_data)
    return transformed_data


def prepare_data(training_data, new_data, transform_to_angle=False):
...
def transform_to_angle(dataframe, feature_1="PCR_02", feature_2="PCR_06"):
